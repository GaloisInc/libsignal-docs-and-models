...
- **PQ security as UAKE with PQ signature.** We model PQ security as "UAKE security with compromised DH key exchange." This allows us to reuse our UAKE notion, but it's not quite the desired statement of security. In a PQ setting, PQXDH is intended to be resistant to harvest-now-decrypt-later (HNDL) attacks, *not* a general authenticated key exchange notion. The difference is that, although key secrecy should be maintained, authenticity doesn't need to be. However, UAKE doesn't allow us to argue about key secrecy in isolation. Moreover, in order to show the authenticity part of UAKE, we must assume that the signature scheme is PQ secure, which is not the case for PQXDH in practice. The way to fix this would be to use a dedicated HNDL security notion here, rather than UAKE, and omit the hypothesis that the signature scheme is secure.
- **KDF modeled as a PRF in non-PQ security.** We currently model the KDF as a PRF in both the PQ and non-PQ security theorems. The previous CryptoVerif analysis uses a random oracle to model the KDF in the non-PQ case, so it may be necessary for us to do this as well in order to complete the proof.
