...
- **SUF-CMA signature (not EUF-CMA).** Since UAKE is a transcript-matching-style definition, our security theorems are subject to harmless but definition-breaking "no-match" attacks on the signature scheme. See {Informal.citet LS17}[] for a reference on attacks of this kind.
