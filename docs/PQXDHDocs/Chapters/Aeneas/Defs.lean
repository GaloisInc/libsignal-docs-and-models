...
- **Inherited Spec simplifications.** The Spec theorems this reduces to are still sorry-backed and carry their own simplifications (SUF-CMA signatures, the KDF as a PRF); see the Specification chapter. In particular, the PQ theorem inherits the modeling of PQ security as "UAKE security with compromised DH key exchange," which assumes a PQ-secure signature scheme; see the "PQ security as UAKE with PQ signature" bullet there. The KDF-as-a-PRF modeling may also change: the previous CryptoVerif analysis models the KDF as a random oracle in the non-PQ case, and completing the Spec proofs may require the same; see the "KDF modeled as a PRF in non-PQ security" bullet there.
