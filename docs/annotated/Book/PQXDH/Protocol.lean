...
Using these keys, Bob calculates _PQKEM-DEC(PQPKB, CT)_ as the shared secret _SS_{galoisnote}[{libsignal "rust/protocol/src/pqxdh.rs#L368-L373"}[]] and repeats the _DH_ and _KDF_ calculations from the previous section to derive _SK_{galoisnote}[{libsignal "rust/protocol/src/pqxdh.rs#L375"}[]], and then deletes the _DH_ values and _SS_ values.
