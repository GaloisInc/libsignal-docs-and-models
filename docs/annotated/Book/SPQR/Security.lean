...
In more detail, *the shared secret generated by the ML-KEM encapsulation function in each epoch acts at the root key of the Double Ratchet mechanism for that epoch*. Notice for example, how the EpochSecret is captured at {spqr "src/v1/chunked/states.rs#L207"}[]:
