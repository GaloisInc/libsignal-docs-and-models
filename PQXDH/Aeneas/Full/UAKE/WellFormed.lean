...
/-!
# Well-formedness of the High-fidelity Extraction's UAKE Schemes
...
`ECGroupModel` and `KemPairModel` (as in `Security.lean`) and transfer the
honest run to the Spec model along the party simulations. The supporting
lemmas live in `WellFormedLemmas.lean`.
-/
...
/-- The extracted T=Bob scheme is well-formed: both parties output only at
  ...
  * MODEL IDEALIZATION: the round-count condition assumes the clean-group
    model `ECGroupModel` and the KEM pairing `KemPairModel` (see
    `SecurityDefs.lean`), the totality hypotheses, and the correctness
    hypotheses, since an honest run completes only when no opaque primitive
    fails. -/
theorem uakeInitiator_wellFormed
  ...
  rw [setup_toSpec P gen privEnc hM.keygen_eq msg] at hsetup
  simp only [support_map, Set.mem_image, Prod.exists, Prod.map_apply,
    Prod.mk.injEq] at hsetup
  obtain ⟨uk₀, tk₀, hsetup₀, huk, htk⟩ := hsetup
  subst huk htk
  rw [Party.runHonest_transport
    (initiator_sim P gen privEnc hM.keygen_eq hM.agree_eq hencTotal hkdfTotal)
    (recipient_sim P gen privEnc hM.keygen_eq hM.agree_eq hM.canonical_eq hK.keygen_eq
      hkdfTotal hasOPK)] at hrun
  exact PQXDH.runHonest_length_initiator (specParams P F gen) msg hasOPK hsig hkem haead
    hsetup₀ hrun
...
theorem uakeRecipient_wellFormed
  ...
  subst huk htk
  rw [Party.runHonest_transport
    (recipientNoConfirm_sim P gen privEnc hM.keygen_eq hM.agree_eq hM.canonical_eq
      hK.keygen_eq hkdfTotal hasOPK)
    (initiatorNoConfirm_sim P gen privEnc hM.keygen_eq hM.agree_eq hencTotal
      hkdfTotal)] at hrun
  exact PQXDH.runHonest_length_recipient (specParams P F gen) msg hasOPK hsig hkem haead
    hsetup₀ hrun
